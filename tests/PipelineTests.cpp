...
TEST_CASE(Pipeline_2DSmokeRenderTriangleToContext) {
    ...
    pipe.RenderTriangle(
        Engine::WorldVertex2D<double>({-0.5, -0.5}, 2.0),
        Engine::WorldVertex2D<double>({ 0.5, -0.5}, 2.0),
        Engine::WorldVertex2D<double>({ 0.0,  0.5}, 2.0)
    );
...
TEST_CASE(Pipeline_3DDepthBufferAndViewportControls) {
    Tests::BufferContext ctx(20, 10);
    Engine::ContextGraphics<double> graphics(ctx);
    Engine::ScalableInterpolator<double> interp;
    ...
    const Engine::WorldVertex3D<double> a({-0.7, -0.7, 0.0}, 2.0);
    const Engine::WorldVertex3D<double> b({ 0.7, -0.7, 0.0}, 2.0);
    const Engine::WorldVertex3D<double> c({ 0.0,  0.7, 0.0}, 2.0);

    pipe.RenderTriangle(a, b, c); // near

    const Engine::WorldVertex3D<double> af({-0.7, -0.7, 0.5}, 9.0);
    const Engine::WorldVertex3D<double> bf({ 0.7, -0.7, 0.5}, 9.0);
    const Engine::WorldVertex3D<double> cf({ 0.0,  0.7, 0.5}, 9.0);
...
TEST_CASE(Pipeline_3DRenderGeometrySharedVertices) {
    ...
    std::vector<Engine::WorldVertex3D<double>> vertices = {
        Engine::WorldVertex3D<double>({-0.7, -0.7, 0.0}, 3.0),
        Engine::WorldVertex3D<double>({ 0.7, -0.7, 0.0}, 5.0),
        Engine::WorldVertex3D<double>({ 0.0,  0.7, 0.0}, 7.0)
    };
