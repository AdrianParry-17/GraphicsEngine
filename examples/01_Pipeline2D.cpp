...
int main() {
    ...
    for (int i = -180; i <= 180; ++i) {
        ...
        pipeline2d.RenderGeometry(
            {
                Engine::WorldVertex2D<double>({-0.5, 0.5}, 2),  // Top Left (color 2)
                Engine::WorldVertex2D<double>({0.5, 0.5}, 4),   // Top Right (color 4)
                Engine::WorldVertex2D<double>({-0.5, -0.5}, 6), // Bottom Left (color 6)
                Engine::WorldVertex2D<double>({0.5, -0.5}, 8)   // Bottom Right (color 8)
            },
        ...
        pipeline2d.RenderGeometry(
            {
                Engine::WorldVertex2D<double>({-0.5, 0.5}, 2),
                Engine::WorldVertex2D<double>({0.5, 0.5}, 4),
                Engine::WorldVertex2D<double>({-0.5, -0.5}, 6),
                Engine::WorldVertex2D<double>({0.5, -0.5}, 8)
            },
            {0, 1, 2, 1, 2, 3});
        pipeline2d.PopMatrix();
